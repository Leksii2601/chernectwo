...
describe('Eternal Calendar Logic - OCU_RULES.json', () => {
    ...
    describe('Scenario B: Movable Feast Shift', () => {
        ...
        it('should place nday-31 on April 28, 2027 (Pascha March 28 + 31 days)', () => {
            const pascha2027 = getOrthodoxPascha(2027);
            const expected = formatDate(pascha2027);

            expect(expected).toBe('2027-03-28'); // Verify Pascha date

            const ndayDate = calculateNdayDate(2027, 31);
            expect(formatDate(ndayDate)).toBe('2027-04-28');
        });
        ...
        it('should activate correctly on calculated date for 2027', () => {
            const date = new Date(2027, 3, 28); // April 28, 2027
            const result = calculateDynamicReadings(date);
    ...
    describe('Full Database Validation', () => {
        it('should have all 120 rules with valid IDs', () => {
            expect(OCU_RULES.length).toBe(120);
        ...
        it('should have valid readings or matins for all rules', () => {
            ...
            if (emptyRules.length > 0) {
                console.warn('⚠️  Rules without readings:', emptyRules);
            }

            // Some rules might be informational only, so we just warn
            expect(emptyRules.length).toBeLessThan(10);
        });
            ...
            OCU_RULES.forEach(rule => {
                if (rule.triggers.nday) {
                    rule.triggers.nday.forEach(nday => {
                        // Typical range: -70 (Publican & Pharisee) to +70 (Weeks after Pentecost)
                        expect(nday).toBeGreaterThanOrEqual(-100);
                        expect(nday).toBeLessThanOrEqual(100);
                    });
                }
            });
        });
    ...
    describe('Pascha Calculation', () => {
        ...
        it('should calculate Pascha 2027 as March 28', () => {
            const pascha = getOrthodoxPascha(2027);
            expect(formatDate(pascha)).toBe('2027-03-28');
        });
    });
