...
export default async function NewsDetailPage({ params }: { params: Promise<{ id: string, lang: string }> }) {
  ...
  // Translation logic
  const currentLang = (lang?.toUpperCase() || 'UA') as Language;
  const t = (key: string) => translations[currentLang]?.[key] || key;

  const categorySubtitleMap: Record<string, string> = {
    'Публікації': 'news.subtitle_publications',
    'Офіційно': 'news.subtitle_official',
    'Анонси': 'news.subtitle_announcements',
    'Всі новини': 'news.subtitle_all_news',
  };

  const categoryTitleMap: Record<string, string> = {
    'Публікації': 'news.cat_publications',
    'Офіційно': 'news.cat_official',
    'Анонси': 'news.cat_announcements',
    'Всі новини': 'news.cat_all',
  };

  const categorySubKey = categorySubtitleMap[newsItem.category];
  const categoryTitleKey = categoryTitleMap[newsItem.category];
  ...
  return (
    <main className="min-h-screen bg-white">
      <PageHeader
        ...
        backgroundImage={newsItem.innerImage}
      />
