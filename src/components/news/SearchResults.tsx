    ...
    if (results.length === 0) {
        return (
            <div className="max-w-7xl mx-auto px-6 py-12">
                <h2 className="text-2xl font-montserrat text-gray-800 mb-4">No results found for {searchTerm}</h2>
                ...
                {results.map(item => (
                    <Link key={item.id} href={`/news/${item.id}`} className="flex flex-col md:flex-row gap-6 group">
                        ...
                        <div className="flex flex-col justify-start py-2">
                            ...
                            <h3 className="text-xl md:text-2xl font-montserrat font-bold text-gray-900 mb-2 group-hover:text-amber-700 transition-colors">
                                {item.title}
                            </h3>
